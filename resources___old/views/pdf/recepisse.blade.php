...
<body>
        ...
        <br>
        <p>Promoteur de l'entreprise <span style="font-weight: bold;">{{ $entreprise->denomination }}</span> exerçant dans le domaine de <span style="font-weight: bold;">  {{ getlibelle($entreprise->secteur_activite) }}</span> a souscrit sur la plateforme de BRAVE WOMEN Burkina Faso avec un  projet qui consiste à <span style="font-weight: bold;"> {{ $entreprise->description_du_projet}} .</span> </p> <br>
        ...
        <p style="font-size: x-small;"> Pour toute information contactez nous au : {{ $contact_chef_de_zone }} <br> NB: Ce récépissé constitue le depôt de cadidature aux activités du projet BRAVE WOMEN.</p>
