...
<div class="block full">
            <div class="block-title">
                <div class="row">
                    <div class="col-md-12">
                    <h2>Liste des <strong>baremes</strong></h2>
                    <a href="{{ route('baremes.create') }}" class="btn btn-success pull-right"><span><i class="fa fa-plus"></i></span>Bareme</a>
                    </div>
                </div>
            </div>
<div class="table-responsive">
    <table class="table table-vcenter table-condensed table-bordered listepdf">
        <thead>
                <tr>
                    <th>Critere</th>
                    <th>Valeur inferieure</th>
                    <th>Valeur supérieure</th>
                    <th>Note affectée</th>
                    <th>Action</th>
                </tr>
        </thead>
        <tbody>
            @foreach($baremes as $bareme)
                <tr>
                    <td>{{getlibelle($bareme->valeur_id)}} </td>
                    <td>{{$bareme->valeur_inf}}</td>
                    <td>{{$bareme->valeur_sup}}</td>
                    <td>{{$bareme->note}}</td>
                    <td class="text-center">
                            <div class="btn-group">
                                <a href="{{ route('baremes.edit', $bareme) }}" data-toggle="tooltip" title="Edit" class="btn btn-xs btn-default"><i class="fa fa-pencil"></i></a>
                                <a href="#modal-confirm-delete" onclick="delConfirm({{ $bareme->id }});" data-toggle="modal" title="Supprimer" class="btn btn-xs btn-danger"><i class="fa fa-times"></i></a>
                            </div>
                    </td>
                </tr>
            @endforeach
